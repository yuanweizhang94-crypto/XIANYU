...
def table_names(resources: Any) -> set[str]:
    return set(inspect(resources.engine).get_table_names())


def assert_no_business_tables(resources: Any) -> None:
    assert table_names(resources) <= {"alembic_version"}


def assert_account_tables(resources: Any) -> None:
    assert table_names(resources) == {
        "alembic_version",
        ACCOUNT_TABLE,
        *MESSAGE_TABLES,
        *REPLY_TABLES,
...
def test_revision_relationship_is_empty_baseline() -> None:
    ...
    assert message_revision.down_revision == ACCOUNT_REVISION
    assert message_revision.branch_labels in (None, set())
    assert message_revision.dependencies in (None, ())
    assert reply_revision is not None
...
def test_health_api_does_not_run_migrations_or_add_revisions(tmp_path: Path) -> None:
    app = create_application(
        settings=ApplicationSettings(
            environment="test", database_path=tmp_path / "health-migration.db"
        )
    )
