...
from xianyu_system.core.database import BASELINE_REVISION, get_current_revision, upgrade_database
...
def test_health_endpoint_does_not_run_migrations_or_write_database(tmp_path: Path) -> None:
    ...
    assert before_revision == after_revision == BASELINE_REVISION
    assert before_tables == after_tables == {"alembic_version"}
