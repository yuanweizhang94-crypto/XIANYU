# CHG-0033 Read-Only AI Auto-Reply Preflight
...
`BRANCH=feat/CHG-0033-ai-auto-reply-live-canary-yilong`
...
## Commander Authorized Sender-Free Provider Connection Test

`COMMANDER_AUTHORIZED_SINGLE_PROVIDER_CONNECTION_TEST=true`

`PROVIDER_CONNECTION_TEST_FUNCTION=common.services.ai_provider_service.test_ai_connection`

`PROVIDER_CONNECTION_TEST_PROVIDER_TYPE=openai_compatible`

`PROVIDER_CONNECTION_TEST_MODEL_NAME=qwen-plus`

`PROVIDER_CONNECTION_TEST_BASE_URL_SOURCE=DEFAULT_AI_BASE_URL`

`PROVIDER_CONNECTION_TEST_BASE_URL_VALUE_PRINTED=false`

`PROVIDER_CONNECTION_TEST_API_KEY_SUPPLIED_PROCESS_MEMORY=true`

`PROVIDER_CONNECTION_TEST_API_KEY_VALUE_PRINTED=false`

`PROVIDER_CONNECTION_TEST_CREDENTIAL_LOGGED=false`

`PROVIDER_CONNECTION_TEST_CREDENTIAL_PERSISTED=false`

`PROVIDER_CONNECTION_TEST_CREDENTIAL_COMMITTED=false`

`PROVIDER_CONNECTION_TEST_INVOCATIONS=1`

`PROVIDER_CONNECTION_TEST_SECOND_INVOCATION_BLOCKED=false`
...
`PROVIDER_CONNECTION_TEST_ACCOUNT_CONFIG_MUTATED=false`

`PROVIDER_CONNECTION_TEST_REPO_WRITE_FROM_PROBE=false`
...
`RUNNING_XIANYU_SERVICES_OBSERVED=true`
