...
Before commit, rollback is deletion of this active Change record, its evidence, tests, and generated state refresh from the isolated CHG-0030 worktree only. No production runtime, database, account state, adapter runtime, or `D:/xianyu` dirty worktree state is modified.
