...
from scripts.generate_state import write_project_state
...
def check_capability_registry(root: Path) -> None:
    ...
    bad_statuses = [str(item.get("id")) for item in capabilities if item.get("status") != "planned"]

    if bad_statuses:

        raise VerificationError(f"capabilities must remain planned: {', '.join(bad_statuses)}")
...
def run_pytest(root: Path) -> None:
    ...
    summary = {"summary": "pytest passed during verify_repository.py", "source": "verify_repository.py"}

    (root / "generated").mkdir(exist_ok=True)

    (root / "generated" / "LAST_TEST_SUMMARY.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")





def generate_and_validate_state(root: Path) -> None:

    output = write_project_state(root)
...
def main() -> int:
    ...
    steps: list[tuple[str, Callable[[Path], None]]] = [
        ...
        ("generate and validate project state", generate_and_validate_state),
